...
def detect_Aruco(img,path_to_image):  #returns the detected aruco list dictionary with id: corners
    aruco_list = {}
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    if path_to_image=='Image1.jpg' or path_to_image=='Image2.jpg':
    ...
    elif path_to_image=='Image3.jpg' or path_to_image=='Image4.jpg':
        aruco_dict = aruco.getPredefinedDictionary(aruco.DICT_5X5_1000)
    elif path_to_image=='Image5.jpg':
...
def mark_Aruco(img, aruco_list):    #function to mark the centre and display the id
    ...
    for key in key_list:
        ...
        cv2.putText(img, str(key), (int(centre[0] + 20), int(centre[1])), font, 1, (255,0,), 2, cv2.LINE_AA) # displaying the idno
    return img
...
def calculate_Robot_State(img,aruco_list):  #gives the state of the bot (centre(x), centre(y), angle)
    ...
    for key in key_list:
        ...
        cv2.putText(img, str(angle), (int(centre[0] - 80), int(centre[1])), font, 1, (255,0,0), 2, cv2.LINE_AA)
